...
var covid_summary, table, countries = {};

mapboxgl.accessToken = 'pk.eyJ1Ijoid2xpbnRvbjEiLCJhIjoiY2toY2QxZjN1MDBicDM1cXQ3bGlrc3luMSJ9.6bgXKec_kh-VRDO-eJBk2g';
...
$(document).ready(function () {
  $.get("https://api.covid19api.com/summary", function (covid_summary, status) {

    document.getElementById('worldCases').textContent = `Global Cases: ${covid_summary.Global.TotalConfirmed}`

    document.getElementById('worldDeaths').textContent = `Global Deaths: ${covid_summary.Global.TotalDeaths}`

    countries.type = "FeatureCollection";
    countries.features = [];

    covid_summary.Countries.forEach(function (country, i) {

      let geo_country = { "type": "", geometry: {}, properties: {} };
      geo_country.type = "Feature";
      geo_country.geometry.type = "Point";

      if (geo_coordinates[country.CountryCode] != null) {

        geo_country.geometry.coordinates = geo_coordinates[country.CountryCode].reverse();
      } else {
        geo_country.geometry.coordinates = [0, 0];
      }
      geo_country.properties.country = country.Country;
      geo_country.properties.TotalConfirmed = country.TotalConfirmed;
      geo_country.properties.TotalDeaths = country.TotalDeaths;
      geo_country.properties.id = i;
     
      
      

      countries.features.push(geo_country);

      let country_code = country.CountryCode.toLowerCase();
      let flag = `<figure class="cov19u-avatar"><img src="https://worldcoronastatistics.com/wp-content/plugins/covid-19-ultimate/assets/svg/flags/` + country_code + `.svg"></figure>`;
      country.Country = flag + country.Country;

    });

    table = $('#covid_table').DataTable({
      data: covid_summary.Countries,
      pageLength: 25,
      columns: [
        { "data": "Country" },
        { "data": "TotalConfirmed" },
...
function addMarkers(data) {
  ...
  data.features.forEach(function (marker) 
  {
    // console.log(marker)
let color = ''
    if (marker.properties.TotalConfirmed > 1000000) {
      color = "red"
    } else if (marker.properties.TotalConfirmed > 500000) {
      color = "orange"
    } else if (marker.properties.TotalConfirmed > 100000) {
      color = "yellow"
    } else {
      color = "green"
    }
console.log(color)
    ...
    // var newMark = new mapboxgl.Marker(el, { offset: [0, -23], color: color })
...
function buildLocationList(data) {
  data.features.forEach(function (country, i) {
...
function flyToCountry(currentFeature) {
  map.flyTo({
...
function createPopUp(currentFeature) {
  ...
  var popup = new mapboxgl.Popup({ closeOnClick: false })
    ...
    .setHTML('<h4>' + currentFeature.properties.country + '</h4>')
    .addTo(map);
}
